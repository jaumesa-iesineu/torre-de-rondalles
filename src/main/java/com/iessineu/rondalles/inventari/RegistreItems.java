...
public class RegistreItems {
    ...
    public static RegistreItems get() {
        if (instancia == null) instancia = new RegistreItems();
        return instancia;
    }
    ...
    private Iterable<String[]> llegirFitxer(String ruta) {
        ...
        if (in == null) throw new RuntimeException("No s'ha trobat el fitxer: " + ruta);
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(in, StandardCharsets.UTF_8))) {
            ...
            while ((linia = br.readLine()) != null) {
                ...
                for (int i = 0; i < camps.length; i++) camps[i] = camps[i].trim();
