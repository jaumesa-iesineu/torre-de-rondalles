...
public class Inventari {
    ...
    // treu un item de la motxilla
    public void treu(Item item) {
        items.remove(item);
    }

    // equipa una armadura al seu slot i la treu de la motxilla
    public void equipaArmadura(Armadura armadura, Jugador jugador) {
        ...
        if (anterior != null) items.add(anterior); // la peça anterior torna a la motxilla
    ...
    public void equipaArma(Arma arma, Jugador jugador) {
        if (armaEquipada != null) items.add(armaEquipada); // l'arma anterior torna a la motxilla
        armaEquipada = arma;
        items.remove(arma);
        arma.aplicaEfecte(jugador);
    }

    // recalcula la defensa total sumant totes les peces equipades
    ...
    // pes total: items a la motxilla + equipament
    public int pesTotal() {
    ...
    // càlcul de penalització de velocitat segons el pes
